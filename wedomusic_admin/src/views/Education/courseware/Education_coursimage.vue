   ...
   <div class="content content_model content_model_add">
      <div class="home_text">新增板书
       ...
       <a class="table_a">标题:</a>
        <Input style="width: 300px"  placeholder="请输入板书标题" v-model="cours_name"></Input>
       </div>
           <div class="input_div" style="margin-bottom:30px;">
       <a class="table_a">顺序码:</a>
       <Select style="width:300px !important;" v-model="cours_sort">
...
<script>
  ...
  export default {
    data () {
      return {
        ware_id:localStorage.getItem('ware_id'),
    ...
    getmain(){   // 获取详情
      ...
      .then(res=>{
        ...
        let i = res.data.data 
        this.cours_name = i.name
        this.cours_sort = i.sort.toString()
        this.res_url = i.res_url
        this.lesson = i.res_url
        $('#courseware_item_content').css('background-image','url("'+ i.res_url +'")');
      })
      .catch(err=>{
    ...
    not_null(){   // 判断
      ...
      }else if(this.cours_sort == ''){
        this.$Message.warning('请选择顺序码')
      }else if(this.lesson==""){
        this.$Message.warning('请上传课件板书')
      }else{
        this.add_cours()
      }
    },
    ...
    add_cours(){   // 新增+修改
      this.$axios({
        ...
        data: {
          token: localStorage.getItem('token'),
            ...
            name:this.cours_name,
            courseware_id:this.courseware_id,
            resource_category_id:1,  // 1板书 2视频 3跟铺
            res_url:this.lesson,
            xml_url:"",
            json_url:"",
            sort:this.cours_sort,
          },
          transformRequest: [function (data) {
            let ret = ''
            for (let it in data) {
      ...
      .then(res=>{
            // console.log(res.data.data)
            this.$router.push({path:'/education/coursechildlist' +'?'+ this.ware_id})
    ...
    upload(id){   // 上传1
      ...
      if(ext!='png'&&ext!='jpg'){
        this.$Message.warning('上传"jpg"或"png"图片')
        return;
      }
