  ...
  <!--扩展提高-->
   <div v-if="show_add">
     <div class="input_div" style="margin-top:20px;">
       ...
       <RadioGroup v-model="vertical" vertical @on-change='change_video_url'>
            <Radio label="1">
                <span>已有视频课程</span>
            ...
            <Radio label="2">
                <span>上传新视频</span>
                <Tag v-for="item in up_video_item" :key="item" :name="item"  closable @on-close="video_close">{{ item }}</Tag>
        ...
        <div style="padding-top:20px">
          <div class="input_div">
            ...
            <CheckboxGroup v-model="video_arrs" @on-change='add_video_arr'>
                <Checkbox v-for="res in res_url_arr" :key="res.name" :label="(res)">{{res.name}}</Checkbox>
            </CheckboxGroup>
  ...
  methods:{
    ...
    get_main(){    // 获取详情
      ...
      .then(res=>{
          ...
          // 跟谱
          if(i.textbook_ids){
            for(let a=0;a<i.textbook_ids.length;a++){    
          ...
          if(i.video_url){
            for(let a=0;a<i.video_url.length;a++){
              if(i.video_url[a].name){   // 已有视频
                ...
                let res = i.video_url[a]
                let ress = res.split('/')
                this.up_video_item.push(ress[4])
              }
    ...
    */
    ...
    show_lows(){  // 扩展提高
    console.log(this.show_low[0])
      if(this.show_low[0] == '1'){
          this.show_add = true
      }else{
        this.show_add = false
      }
      if(this.show_low[0]==0){
        this.show_low = ['1']
        this.show_add = true
      }
    },
    ...
    add_aork(){   // 添加
      ...
      .then(res=>{
        ...
        else if(res.data.message=='textbook_ids is null 或 不是json'){
          this.$Message.warning('请选择跟谱练习')
        }
      })
    },
        ...
        key:key         // 定义文件名
          ...
          if(oReq.readyState == 4){
            if(oReq.status==200){ 
              ...
              that.video_item.push((that.lesson).toString())   // 新视频上传  （后端）
              that.up_video_item.push((that.lesson).toString())  // 展示
              console.log(that.video_item)
              that.$Loading.finish();
            }else{
